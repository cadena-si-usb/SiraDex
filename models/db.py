...
db.define_table('CAMPO_CATALOGO',
    ...
    Field('tipo_cat',type='string', length=256,
           requires = [IS_IN_SET(tipo_campos, zero="Seleccione...", error_message = "Seleccione un tipo de campo.")],
           widget = SQLFORM.widgets.options.widget),
    Field('nombre', type='string', length=64,
           requires = [IS_NOT_EMPTY(error_message='El nombre del campo no puede quedar vacio.'),
                       IS_MATCH('([A-Za-z])([A-Za-z0-9" "])*', error_message="El nombre del campo debe comenzar con una letra y no puede contener caracteres especiales.")]),
